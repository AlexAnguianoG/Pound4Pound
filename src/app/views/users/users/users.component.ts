...
export class UsersComponent implements OnInit {
  ...
  Role: new FormControl('',Validators.compose([Validators.required, Validators.min(4)]))
});
...
Role: new FormControl('',Validators.compose([Validators.required, Validators.min(4)]))
  ...
  loadUserToEdit(UserId: number, Username: string, Password: String, Email: string, Role: string) {
    //let update
      this.registerFlag = false;
      this.userService.getUserById(UserId)
           .subscribe(update => {
               ...
               this.updateForm.patchValue({  Username: Username, Password: Password, Email: Email, Role: Role});
